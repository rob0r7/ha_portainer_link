...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    enable_container_buttons = conf.get("enable_container_buttons", True)
    enable_stack_buttons = conf.get("enable_stack_buttons", False)
    ...
    for container in containers:
        ...
        # Create individual container buttons (respect toggle)
        if enable_container_buttons:
        ...
        # Add stack-level buttons only once per stack (respect toggle)
        if enable_stack_buttons and stack_info.get("is_stack_container"):
...
class RestartContainerButton(ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            _LOGGER.warning("Failed to restart container %s: %s", self._container_name, e)


class PullUpdateButton(ButtonEntity):
    """Button to pull latest image for a Docker container."""

    def __init__(self, name, api, endpoint_id, container_id, stack_info, entry_id):
        self._attr_name = f"{name} Pull Update"
        self._container_name = name
        self._api = api
        self._endpoint_id = endpoint_id
        self._container_id = container_id
    ...
    async def async_press(self) -> None:
        await self._ensure_container_bound()
        try:
            await self._api.pull_update(self._endpoint_id, self._container_id)
        except Exception as e:
            _LOGGER.warning("Failed to pull update for container %s: %s", self._container_name, e)


class StackStopButton(ButtonEntity):
    def __init__(self, stack_name, api, endpoint_id, stack_info, entry_id):
        self._attr_name = f"{stack_name} Stop"
        self._stack_name = stack_name
        self._api = api
        self._endpoint_id = endpoint_id
        self._stack_info = stack_info
        self._entry_id = entry_id
        self._attr_unique_id = _build_stable_unique_id(entry_id, endpoint_id, stack_name, stack_info, "stack_stop")

    async def async_press(self) -> None:
        try:
            await self._api.stop_stack(self._endpoint_id, self._stack_name)
        except Exception as e:
            _LOGGER.warning("Failed to stop stack %s: %s", self._stack_name, e)


class StackStartButton(ButtonEntity):
    def __init__(self, stack_name, api, endpoint_id, stack_info, entry_id):
        self._attr_name = f"{stack_name} Start"
        self._stack_name = stack_name
        self._api = api
        self._endpoint_id = endpoint_id
        self._stack_info = stack_info
        self._entry_id = entry_id
        self._attr_unique_id = _build_stable_unique_id(entry_id, endpoint_id, stack_name, stack_info, "stack_start")

    async def async_press(self) -> None:
        try:
            await self._api.start_stack(self._endpoint_id, self._stack_name)
        except Exception as e:
            _LOGGER.warning("Failed to start stack %s: %s", self._stack_name, e)


class StackUpdateButton(ButtonEntity):
    def __init__(self, stack_name, api, endpoint_id, stack_info, entry_id):
        self._attr_name = f"{stack_name} Update"
        self._stack_name = stack_name
        self._api = api
        self._endpoint_id = endpoint_id
        self._stack_info = stack_info
        self._entry_id = entry_id
        self._attr_unique_id = _build_stable_unique_id(entry_id, endpoint_id, stack_name, stack_info, "stack_update")

    async def async_press(self) -> None:
        try:
            await self._api.update_stack(self._endpoint_id, self._stack_name)
        except Exception as e:
            _LOGGER.warning("Failed to update stack %s: %s", self._stack_name, e)
